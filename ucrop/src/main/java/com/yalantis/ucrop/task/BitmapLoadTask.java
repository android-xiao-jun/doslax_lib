    ...
    protected BitmapWorkerResult doInBackground(Void... params) {
       ...
       /*
        try {
            ...
            Log.e(TAG, "Bitmap could not be decoded from the Uri: [" + mInputUri + "]", e);
        }
        */
        ...
        while (!decodeAttemptSuccess) {
            try {
                ...
                if (stream != null) {
                    ...
                    try {
                        breakPoint:{
                            short first = (short) stream.read();
                            short two = (short) stream.read();
                            int bit = (int) first << 8 | two;
                            if (bit == 0xFFD8) {//jpg
                                break breakPoint;
                            }
                            ...
                            isSaveToJpg = true;
                            /*
                            short three = (short) stream.read();
                            short four = (short) stream.read();

                            int bit2 = (bit << 8) | three;
                            if (bit2 == 0x474946) {//gif
                                isSaveToJpg = true;

                                break breakPoint;
                            }

                            int bit3 = (three << 8) | four;
                            if (bit3 == 0x89504E47) {//png
                                isSaveToJpg = true;
                            }
                             */
                        }
        ...
        if (isSaveToJpg){
            try {//将gif 的一张图片截取下来，另外存一个文件
                File file = BitmapLoadUtils.saveImageToFilePath(mOutputUri.getPath(), decodeSampledBitmap);
                mInputUri = Uri.fromFile(file);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
