...
public class TimeUtils {
     ...
     * 瞬间列表：
     * 1、时间格式：
     *     1）今天以内的的显示 hh:mm   如：10：23
     *     2）今年以内的显示 MM-DD  如：01-10
     *     3）今年以外的显示 YYYY-MM-DD 如：2022-12-10
     */
    public static String formatDate5Formats(long timestamp) {
    try {
        if (isToday(timestamp)) {
            return timeFormat(timestamp, "HH:mm");
        } else if (isThisYear(timestamp)) {
    ...
    public static CharSequence formatSpecialTime3(String dateStr, String pattern) {
        try {
            SimpleDateFormat format = new SimpleDateFormat(pattern, Locale.CHINA);
            format.setTimeZone(TimeZone.getTimeZone("GMT+8"));
            Date date = format.parse(dateStr);
            return formatSpecialTime2(date.getTime());
        }catch (Exception e){
            e.printStackTrace();
        }
        return dateStr;
    ...
    /**
     * 获取今日凌晨的时间戳  yyyy-MM-dd 00:00:00
     * @return
     */
    ...
    public static String formatSecondsSmart(int totalSeconds) {
    ...
    public static int dateToAge(String birthDateStr){
