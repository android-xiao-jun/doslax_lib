...
public class CustomCameraView extends FrameLayout implements LifecycleObserver {
    ...
    //切换摄像头按钮的参数
    private final int iconSrc;        //图标资源
    private final int iconLeft;       //左图标
    private final int iconRight;      //右图标
    private final int duration;      //录制时间
    private final int showOnlyOne;   //展示仅看一次
    private long recordTime = 0;

    private boolean isClickConform;//是否是点击确定
    ...
    public CustomCameraView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        duration = a.getInteger(R.styleable.CustomCameraView_duration_max, 10 * 1000 * 6);       //没设置默认为10s x 6
        showOnlyOne = a.getInteger(R.styleable.CustomCameraView_show_only_one_view, 0);       //没设置默认不展示仅看一次View
        a.recycle();
        initView();
    }
    ...
    void onPause() {
        //如果还在播放视频 则停止播放
        if (!isClickConform && mCameraView.getMode() == Mode.VIDEO) {
            stopVideoPlay();
            resetState();
        }
    }
    ...
    public void initView() {
        ...
        View view;
        if (showOnlyOne == 1) {
            view  = LayoutInflater.from(mContext).inflate(R.layout.custom_camera_view_only_one, this);
            tvBack = view.findViewById(R.id.tv_back);
        } else {
            view  = LayoutInflater.from(mContext).inflate(R.layout.custom_camera_view, this);
        }
        ...
        mCaptureLayout.setCaptureLisenter(new CaptureListener() {
            ...
            public void recordShort(final long time) {
                ...
                mCaptureLayout.setTextWithAnimation(getResources().getString(R.string.recording_time_is_too_short));
                if (mCameraView.isTakingVideo()) {
                    mCameraView.stopVideo();
                }
            }
        ...
        mCaptureLayout.setTypeLisenter(new TypeListener() {
            ...
            public void confirm() {
                ...
                if (mCameraView.getMode() == Mode.VIDEO) {
                    stopVideoPlay();
                    if (flowCameraListener != null) {
                        flowCameraListener.recordSuccess(videoFile, false);
                    }
                } else {
                    mPhoto.setVisibility(INVISIBLE);
                    if (flowCameraListener != null) {
                        flowCameraListener.captureSuccess(photoFile, false);
                    }
                }
            }
            ...
            public void confirmOnlyOne() {
                Log.e("===z","confirmOnlyOne CCV");
                isClickConform = true;
                if (mCameraView.getMode() == Mode.VIDEO) {
                    stopVideoPlay();
                    if (flowCameraListener != null) {
                        flowCameraListener.recordSuccess(videoFile, true);
                    }
                } else {
                    mPhoto.setVisibility(INVISIBLE);
                    if (flowCameraListener != null) {
                        flowCameraListener.captureSuccess(photoFile, true);
                    }
                }
            }
        });
    ...
    public void setRecordVideoMaxTime(int maxDurationTime) {
        mCaptureLayout.setDuration(maxDurationTime * 1000 * 6);
    }
