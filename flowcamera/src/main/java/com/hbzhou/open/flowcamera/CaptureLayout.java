...
public class CaptureLayout extends FrameLayout {
    ...
    private final int mShowOnlyOne;//是否展示仅看一次
    ...
    private View btn_confirm;         //确认按钮
    private View btn_cancel;          //取消按钮
    ...
    public CaptureLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mShowOnlyOne = a.getInteger(R.styleable.CaptureLayout_show_only_one_view, 0);
        Log.e("===z","mShowOnlyOne=" + mShowOnlyOne);
        a.recycle();
    ...
    private void initView() {
        ...
        btn_capture.setCountDownCallBack(new CaptureButton.CountDownCallBack() {
            @Override
            public void countDown(int countDownTime) {
                txt_tip.setVisibility(View.VISIBLE);
                txt_tip.setText(countDownTime + "S");
            }

            @Override
            public void stop() {
                txt_tip.setVisibility(View.INVISIBLE);
                txt_tip.setText(getResources().getString(R.string.take_image_video));
            }
        });
        ...
        //取消按钮
        LayoutParams btn_cancel_param;
        if (mShowOnlyOne == 1) {
            btn_cancel = new TypeButtonView(getContext(), TypeButtonView.TYPE_SEND_ONLY_ONE);
            btn_cancel_param = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
            btn_cancel_param.setMargins(60, 0, 0, 0);
        } else {
            btn_cancel = new TypeButton(getContext(), TypeButton.TYPE_CANCEL, button_size);
            btn_cancel_param = new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
        ...
        //确认按钮
        LayoutParams btn_confirm_param;
        if (mShowOnlyOne == 1) {
            btn_confirm = new TypeButtonView(getContext(), TypeButtonView.TYPE_SEND);
            btn_confirm_param = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
            btn_confirm_param.setMargins(0, 0, 60, 0);
        } else {
            btn_confirm = new TypeButton(getContext(), TypeButton.TYPE_CONFIRM, button_size);
            btn_confirm_param = new LayoutParams(LayoutParams.MATCH_PARENT, LayoutParams.MATCH_PARENT);
        ...
        //返回按钮
        btn_return = new ReturnButton(getContext(), (int) (button_size / 4.5f));
    ...
    public void resetCaptureLayout() {
        ...
        if (btn_capture.getButtonFeatures() == BUTTON_STATE_ONLY_CAPTURE) {
            txt_tip.setText(getResources().getString(R.string.take_image));
        } else {
            txt_tip.setText(getResources().getString(R.string.take_image_video));
        }
        txt_tip.setVisibility(View.VISIBLE);
    ...
    public void setButtonFeatures(int state) {
        ...
        if (txt_tip != null && state == BUTTON_STATE_ONLY_CAPTURE) {
            txt_tip.setText(R.string.take_image);
        }
    }
