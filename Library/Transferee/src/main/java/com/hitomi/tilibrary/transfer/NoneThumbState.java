...
public class NoneThumbState extends TransferState {
    ...
    public void transferLoad(final int position) {
        ...
        final String imgTargetUrl = transConfig.getTargetUrl(position);
        final TransferImage targetImage = adapter.getImageItem(position);

        File cache = transConfig.getImageLoader().getCache(imgUrl);
        File cacheTargetUrl = transConfig.getImageLoader().getCache(imgTargetUrl);
        if (cache == null && cacheTargetUrl == null) {
            ...
            loadSourceImage(targetImage, progressIndicator, imgTargetUrl, position);
        } else {
            Bitmap bitmap = null;
            String urlString = cache == null ? imgTargetUrl : imgUrl;

            if (cache == null) {
                bitmap = BitmapFactory.decodeFile(cacheTargetUrl.getAbsolutePath());
            } else {
            ...
            loadSourceImage(targetImage, null, urlString, position);
        }
    }
