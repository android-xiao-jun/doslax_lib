 ...
 */
class ThumbnailThumbState extends TransferState {

    ThumbnailThumbState(TransferLayout transfer) {
        super(transfer);
    }
    ...
    public void prepareTransfer(final TransferImage transImage, final int position) {
        final TransferConfig config = transfer.getTransConfig();
        String imgUrl = config.getTargetUrl(position);
        File cache = config.getImageLoader().getCache(imgUrl);
        if (cache == null) return;
        if (ImageUtils.getImageType(cache) == TYPE_GIF) {
            try {
                com.bumptech.glide.Glide.with(transImage).asGif().load(cache).into(transImage);
//                transImage.setImageDrawable(new pl.droidsonroids.gif.GifDrawable(cache.getPath()));
    ...
    public TransferImage transferIn(final int position) {
        ...
        loadThumbnail(config.getTargetUrl(position), transImage, true);
    ...
    public void transferLoad(final int position) {
        final TransferConfig config = transfer.getTransConfig();
        final String imgUrl = config.getTargetUrl(position);
    ...
    public TransferImage transferOut(final int position) {
        ...
        if (position <= originImageList.size() - 1 && originImageList.get(position) != null) {
            ...
            loadThumbnail(config.getTargetUrl(position), transImage, false);
            transfer.addView(transImage, 1);
        }
        return transImage;
    }
