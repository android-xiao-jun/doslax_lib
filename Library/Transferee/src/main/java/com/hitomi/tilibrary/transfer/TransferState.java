                ...
                cacheDir, getDisplaySize(), new ImageProcessor.ImageProcessCallback() {
                    ...
                    public void onSuccess(File file) {
                        if (ImageUtils.getImageType(file) == TYPE_GIF) {
                            try {
                                com.bumptech.glide.Glide.with(targetImage).asGif().load(file).into(targetImage);
//                                targetImage.setImageDrawable(new pl.droidsonroids.gif.GifDrawable(file.getPath()));
     ...
     * @param position
     */
    public void viewTheOriginalImage(final int position,final IProgressIndicator progressIndicator) {
