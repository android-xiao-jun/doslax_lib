...
public final class TransferConfig {
    ...
    /**
     * 获取第一次显示的路径--有缩略图取缩略图，，，没有取原图
     * @param index
     * @return
     */
    public String getTargetUrl(int index) {
        List<String> thumbnailUrlList = getThumbnailUrlList();
        List<String> targetUrlList = getSourceUrlList();
        if (thumbnailUrlList != null && !thumbnailUrlList.isEmpty() && targetUrlList.size() == thumbnailUrlList.size()) {
            String url = thumbnailUrlList.get(index);
            if (!TextUtils.isEmpty(url) && url.startsWith("http")) {
                targetUrlList = thumbnailUrlList;
            }
        }
        return targetUrlList.get(index);
    }
    ...
    public boolean isVideoSource(int position) {
        ...
        String sourceUrl;//FIX BUG 解决判断视频不友好
        if (null != sourceUrlList && pos < sourceUrlList.size()) {
    ...
    public static class Builder {
         ...
         * format: java.lang.String
         */
        public Builder setThumbnailUrlList(List<String> thumbnailUrlList) {
            this.thumbnailUrlList = thumbnailUrlList;
            return this;
        }
        ...
        public TransferConfig create() {
            ...
            config.setThumbnailUrlList(thumbnailUrlList);
