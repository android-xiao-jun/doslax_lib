...
public class BindingRecyclerViewAdapter<T> extends RecyclerView.Adapter<ViewHolder> implements BindingCollectionAdapter<T> {
    ...
    protected ObservableList.OnListChangedCallback<ObservableList<T>> callback;
    ...
    protected boolean isForDataBinding(List<Object> payloads) {
    ...
    protected void tryGetLifecycleOwner() {
        if (lifecycleOwner == null || lifecycleOwner.getLifecycle().getCurrentState() == Lifecycle.State.DESTROYED) {
            lifecycleOwner = Utils.findLifecycleOwner(recyclerView);
        }
    }

    protected static class WeakReferenceOnListChangedCallback<T> extends ObservableList.OnListChangedCallback<ObservableList<T>> {
