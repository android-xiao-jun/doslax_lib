...
public class DownloadHelper {
    ...
    protected ArrayList<String> downloadingQueue = new ArrayList<>();
    ...
    public void start(String url) {
        delete(url);
        if (downloadingQueue.contains(url)) {
            return;
        }
        downloadingQueue.add(url);
    ...
    public void start(List<String> list) {
        ...
        startService("", DownloadConfig.ACTION_START_ALL, newList);
    }
    ...
    public String getLocalFilePathFromUrl(String url) {
        ...
        try {
            ...
            List<DownloadInfo> list = FileDown.getFileDown().getDaoSession().getDownloadInfoDao().queryBuilder().where(DownloadInfoDao.Properties.Url.eq(url)).list();
            if (list != null && !list.isEmpty()) {
                DownloadInfo info = list.get(0);
                if (info == null) {
                    localPath = null;
                } else {
